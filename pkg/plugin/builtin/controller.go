...
var (
	_ plugin.Base              = &ReplaceControllerPlugin{}
	_ plugin.GenericSubcommand = &ReplaceControllerPlugin{}
)

type ReplaceControllerPlugin struct {
}

func (ReplaceControllerPlugin) Name() string {
	return "replace-controller"
}

func (ReplaceControllerPlugin) Version() string {
	return "v2.0.0"
}

func (ReplaceControllerPlugin) SupportedProjectVersions() []string {
	return []string{"2"}
}

func (ReplaceControllerPlugin) Run(state plugin.State) error {
	controllerPath := filepath.Join("controllers", strings.ToLower(u.Resource.Kind)+"_controller.go")
	if state.Has(controllerPath) {
		return fmt.Errorf("cannot replace %s: file already exists", controllerPath)
	}

	if err := state.Delete(controllerPath); err != nil {
		return err
	}
