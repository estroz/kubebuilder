...
func (p *createAPISubcommand) Run(fs machinery.Filesystem) error {
	if err := p.config.DecodePluginConfig(plugin.KeyFor(Plugin{}), &p.pconfig); err != nil {
...
func (p *createAPISubcommand) GetScaffolder() (cmdutil.Scaffolder, error) {
	...
	return scaffolds.NewAPIScaffolder(p.config, p.resource, p.force, p.pconfig.WithKustomize, plugins), nil
}
