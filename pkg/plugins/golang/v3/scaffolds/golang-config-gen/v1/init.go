...
- a KubebuilderConfigGen for customizating manifests
- a main.go to run
`
	ctx.Examples = fmt.Sprintf(`  # Scaffold a project using the apache2 license with "The Kubernetes authors" as owners
  %[1]s init --domain example.org --license apache2 --owner "The Kubernetes authors"

  # Scaffold a project that uses KubebuilderConfigGen indirectly as a kustomize transformer
  %[1]s init --with-kustomize
`, ctx.CommandName)
...
func (p *initSubcommand) Run(fs machinery.Filesystem) error {
	if err := p.config.EncodePluginConfig(plugin.KeyFor(Plugin{}), p.pconfig); err != nil {
		return fmt.Errorf("error encoding plugin config: %v", err)
	}
...
func (p *initSubcommand) GetScaffolder() (cmdutil.Scaffolder, error) {
	...
	return scaffolds.NewInitScaffolder(p.config, p.license, p.owner, p.pconfig.WithKustomize), nil
}
