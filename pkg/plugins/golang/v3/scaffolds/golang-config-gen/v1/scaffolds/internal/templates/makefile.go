...
// TODO(estroz): add (un)install targets, which are necessary for `make run`.
// This require config-gen to be able to generate CRDs only.
...
deploy: kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	$(KUSTOMIZE) build --enable-alpha-plugins config/default | kubectl apply -f -

undeploy: kustomize ## Undeploy controller from the K8s cluster specified in ~/.kube/config.
	$(KUSTOMIZE) build --enable-alpha-plugins config/default | kubectl delete -f -
...
undeploy: ## Undeploy controller from the K8s cluster specified in ~/.kube/config.
	kubebuilder alpha config-gen ./kubebuilderconfiggen.yaml | kubectl delete -f -
...
kustomize: ## Download kustomize locally if necessary.
	$(call go-get-tool,$(KUSTOMIZE),sigs.k8s.io/kustomize/kustomize/v4@{{ .KustomizeVersion }})
	@# Now make sure the config-gen plugin is set up.
